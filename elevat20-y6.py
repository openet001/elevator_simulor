...
class Elevator:
    def __init__(self, eid: int, allowed_floors: List[str], max_capacity: int):
        ...
        self.status = "idle"  # up, down, opening, closing, idle
    ...
    def get_passenger_text(self):
        return f"{len(self.passengers)}/{self.max_capacity}"

    def get_target_floors_text(self):
        if not self.target_floors:
            return "无"
        return "→".join([f for f in self.target_floors])
...
class ElevatorSystemGUI:
    def __init__(self, master):
        ...
        self.floors_down_var = tk.IntVar(value=2)  # 修改默认值为2
    ...
    def setup_simulation(self):
        ...
        # 加宽画布宽度，避免与楼层编号重叠
        canvas_width = 100 + n_elev * 100
        self.canvas = tk.Canvas(self.sim_frame, width=canvas_width, height=40*len(self.floors)+40, bg="white")
    ...
    def is_peak(self):
        current_min = self.time % 1440  # 一天总分钟数
        morning_start, morning_end = self.peak_periods["morning"]
        evening_start, evening_end = self.peak_periods["evening"]
        if (morning_start <= current_min < morning_end) or (evening_start <= current_min < evening_end):
            return True
        else:
            return False
    ...
    def generate_passengers(self):
        ...
        for floor in self.floors:
            if random.random() < base_rate:
                ...
                t = self.time % 1440
                peak = False
                if (self.peak_periods["morning"][0] <= t < self.peak_periods["morning"][1]):
                    if floor in self.floors[-(len(self.floors)-self.floors.index("0")-1):]:  # 地下楼层
                        ...
                        direction = "up"
                        peak = True
                elif (self.peak_periods["evening"][0] <= t < self.peak_periods["evening"][1]):
    ...
    def step_elevators(self):
        ...
        for elevator in self.elevators:
            ...
            # 上客
            floor_queue = deque()  # 初始化为空队列
            if elevator.current_floor in self.waiting_passengers:  # 检查当前楼层是否有等待的乘客
            ...
            if elevator.passengers:
                ...
                if elevator.direction == "up":
                    ...
                    # 如果电梯当前没有移动，但有乘客，可能是刚到达目标楼层，准备开门
                ...
                # 响应等待人数多/等待时间久的楼层
                candidates = []
                for f in elevator.allowed_floors:  # 遍历电梯允许的楼层
                    if f in floor_waits and floor_waits[f][1] > 0 and f not in elevator_targets:
                        candidates.append((f, floor_waits[f][1], floor_waits[f][0]))
            ...
            # 检查是否需要更新target_floors
            # 如果电梯已经到达所有目标楼层，清空target_floors并进入空闲状态
            if elevator.target_floors and elevator.current_floor in elevator.target_floors:
                # 移除已到达的目标楼层
                while elevator.target_floors and elevator.current_floor == elevator.target_floors[0]:
                    elevator.target_floors.popleft()
                # 如果没有更多目标楼层，设置状态为idle
    ...
    def draw_static(self):
        ...
        for i, floor in enumerate(self.floors):
            y = 40 + i*40
            self.canvas.create_line(60, y, 60 + len(self.elevators)*100, y, fill="#ccc")
            self.canvas.create_text(40, y, text=floor, font=("Arial", 12))
        for eid in range(len(self.elevators)):
            x = 60 + eid*100
            self.canvas.create_rectangle(x, 40, x+100, 40+len(self.floors)*40, outline="#888")
    ...
    def draw_elevators(self):
        for eid, elevator in enumerate(self.elevators):
            try:
                ...
                # 如果电梯当前楼层不在floors列表中(理论上不应该发生)，则跳过
                continue
            x = 60 + eid*100
            
            # 绘制电梯状态信息
            status_text = elevator.get_status_text()
            passenger_text = elevator.get_passenger_text()
            target_text = elevator.get_target_floors_text()
            
            # 在电梯矩形内显示状态信息
            self.canvas.create_rectangle(x+5, y-30, x+95, y+10, fill="skyblue", outline="black", width=2)
            
            # 显示电梯状态、人数和目标楼层
            info_text = f"电梯{eid}\n{status_text}\n{passenger_text}\n目标:{target_text}"
            self.canvas.create_text(x+50, y-10, text=info_text, font=("Arial", 8), justify=tk.CENTER)
            
            # 绘制电梯内的乘客方向指示
            if elevator.passengers:
                # 计算乘客方向分布
                up_count = sum(1 for p in elevator.passengers if p.direction == "up")
                down_count = sum(1 for p in elevator.passengers if p.direction == "down")
                
                # 显示方向信息
                direction_text = f"上行:{up_count} 下行:{down_count}"
                self.canvas.create_text(x+50, y+5, text=direction_text, font=("Arial", 6), justify=tk.CENTER)
        # 绘制等待乘客
        for i, floor in enumerate(self.floors):
            q = self.waiting_passengers.get(floor, deque())  # 使用get方法避免KeyError
            y = 40 + i*40
            n = len(q)
            if n > 0:
                # 统计上行和下行乘客数量
                up_count = sum(1 for p in q if p.direction == "up")
                down_count = sum(1 for p in q if p.direction == "down")
                
                # 用不同颜色显示上行和下行乘客
                if up_count > 0:
                    self.canvas.create_text(20, y, text=f"↑{up_count}", fill="green", font=("Arial", 8))
                if down_count > 0:
                    self.canvas.create_text(20, y+10, text=f"↓{down_count}", fill="red", font=("Arial", 8))
    ...
    def update_simulation(self):
        ...
        try:
            ...
            # 更新等待乘客的等待时间
            for floor in list(self.waiting_passengers.keys()):  # 使用list避免在迭代时修改字典
                for p in self.waiting_passengers[floor]:
                    p.waiting_time += 1
            ...
            self.running = False
            self.status_label.config(text=f"仿真出错: {str(e)}")
            messagebox.showerror("错误", f"仿真过程中发生错误: {str(e)}")
