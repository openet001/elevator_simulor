...
class ElevatorSimulator(QMainWindow):
    ...
    def update_simulation(self):
        ...
        if len(self.passengers) == 0:
            self.no_passenger_time += 1
            if self.no_passenger_time >= 60:  # 1分钟无乘客
                # 将所有电梯返回1楼
                for elevator in self.elevators:
                    if not elevator.destination_floors and elevator.current_floor != 1:
        ...
        for elevator in self.elevators:
            ...
            if elevator.door_open:
                ...
                if time.time() - elevator.last_activity_time > 5:  # 5秒开门时间
                    elevator.close_door()
            
            # If elevator is idle, assign a random destination
            if not elevator.destination_floors and not elevator.door_open and len(self.passengers) > 0:
