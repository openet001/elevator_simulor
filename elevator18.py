...
class Passenger:
    def __init__(self, current_floor, destination):
        ...
        self.assigned_elevator = None  # 跟踪分配的电梯
...
class SimulationDisplay(QWidget):
    ...
    def draw_building(self, painter):
        ...
        # Draw floors
        total_floors = self.simulator.total_floors + self.simulator.basement_floors
        floor_height = building_height / (total_floors + 1)  # +1 for better spacing
    ...
    def draw_elevators(self, painter):
        ...
        building_y = 50
        total_floors = self.simulator.total_floors + self.simulator.basement_floors
        floor_height = building_height / (total_floors + 1)
    ...
    def update_simulation(self):
        ...
        for elevator in self.elevators:
            ...
            if elevator.door_open:
                ...
                # 乘客上电梯
                waiting = self.waiting_passengers.get(elevator.current_floor, [])
                if waiting:
                    # 根据方向筛选乘客
                    if elevator.direction == 1:  # 上行
                        to_board = [p for p in waiting if p.direction == 1]
                    elif elevator.direction == -1:  # 下行
                        to_board = [p for p in waiting if p.direction == -1]
                    else:  # 空闲
                        to_board = waiting[:elevator.max_capacity - len(elevator.passengers)]
                    
                    # 尝试让乘客进入电梯
                    for p in to_board[:]:
                        if elevator.board_passenger(p):
                            waiting.remove(p)
                            p.in_elevator = True
                
                # 检查是否需要关门
                if time.time() - elevator.last_activity_time > 5:
                    elevator.close_door()
            
            # 如果电梯空闲且有乘客等待，分配新任务
            if not elevator.destination_floors and not elevator.door_open and self.passengers:
                # 查找最近的等待乘客
                for floor, passengers in sorted(self.waiting_passengers.items(),
                                             key=lambda x: abs(x[0] - elevator.current_floor)):
                    if floor in elevator.allowed_floors and passengers:
