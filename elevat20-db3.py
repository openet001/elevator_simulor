...
class Elevator:
    def __init__(self, eid: int, allowed_floors: List[str], max_capacity: int):
        ...
        self.status = "idle"
        self.current_y = 40 + allowed_floors.index(self.current_floor) * 60  # 电梯Y坐标（调整为60像素间距）
...
class ElevatorSystemGUI:
        ...
        for elevator in self.elevators:
            ...
            if elevator.passengers:
                ...
                candidates = [(f, floor_waits[f][1], floor_waits[f][0]) for f in elevator.allowed_floors
                              if floor_waits[f][1] > 0 and f not in elevator.target_floors]
                if candidates:
            ...
            if elevator.direction in ["up", "down"]:
                ...
                if elevator.direction == "up" and curr_idx > 0 and self.floors[curr_idx-1] in elevator.allowed_floors:
                    ...
                    elevator.current_floor = self.floors[curr_idx+1]
                elevator.to_y = 40 + self.floors.index(elevator.current_floor) * 60  # 调整为60像素间距
    ...
    def draw_static(self):
        ...
        for eid in range(n_elev):
            x = 60 + eid * 80
            self.canvas.create_rectangle(x, 40, x + 60, 40 + len(self.floors) * 60,  # 调整为60像素间距
    ...
    def draw_elevators(self):
        ...
        for eid, elevator in enumerate(self.elevators):
            try:
                ...
                y = 40 + self.floors.index(elevator.current_floor) * 60  # 调整为60像素间距
        ...
        for i, floor in enumerate(self.floors):
            ...
            if n > 0:
                # 绘制方向指示和人数
                up_count = len([p for p in q if p.direction == "up"])
                down_count = len([p for p in q if p.direction == "down"])
                
                if up_count > 0:
                    # 上箭头
                    self.canvas.create_polygon(
                        15, y-15, 25, y-25, 35, y-15,
                        fill=self.colors["passenger_wait"], outline="black", width=1, tags="passenger"
                    )
                    self.canvas.create_text(25, y-10, text=str(up_count), font=("Arial", 9),
                                         fill=self.colors["passenger_wait"], tags="passenger")
                
                if down_count > 0:
                    # 下箭头
                    self.canvas.create_polygon(
                        15, y+5, 25, y+15, 35, y+5,
                        fill=self.colors["passenger_wait"], outline="black", width=1, tags="passenger"
                    )
                    self.canvas.create_text(25, y+10, text=str(down_count), font=("Arial", 9),
                                         fill=self.colors["passenger_wait"], tags="passenger")
    ...
    def update_simulation(self):
        ...
        for elevator in self.elevators:
            if hasattr(elevator, 'from_y'):
                self.animate_elevator_movement(elevator)
        # 更新统计信息
        self.update_stats()
        # 计划下一次更新
    ...
    def on_window_resize(self, event):
        ...
        if hasattr(self, 'canvas'):
            self.canvas.config(width=event.width - 320, height=event.height - 120)
            self.draw_static()
    ...
    def toggle_dark_mode(self):
        ...
        if self.dark_mode:
            # 深色模式
            self.colors = {
                "bg_main": "#1e293b", "bg_panel": "#0f172a", "bg_button": "#334155",
                "bg_button_hover": "#475569", "fg_text": "#e2e8f0", "fg_highlight": "#93c5fd",
                "elevator_idle": "#334155", "elevator_up": "#3b82f6", "elevator_down": "#2563eb",
                "passenger_wait": "#f87171", "peak_period": "#f59e0b", "grid_line": "#475569",
                "shadow": "#2d3748", "dark_bg": "#1e293b", "dark_fg": "#e2e8f0"
            }
        else:
            # 浅色模式
            self.colors = {
                "bg_main": "#f0f7ff", "bg_panel": "#ffffff", "bg_button": "#e3f2fd",
                "bg_button_hover": "#bbdefb", "fg_text": "#334155", "fg_highlight": "#1e40af",
                "elevator_idle": "#bfdbfe", "elevator_up": "#3b82f6", "elevator_down": "#2563eb",
                "passenger_wait": "#f87171", "peak_period": "#f59e0b", "grid_line": "#e2e8f0",
                "shadow": "#ddd", "dark_bg": "#1e293b", "dark_fg": "#e2e8f0"
            }
        ...
        self.status_label.configure(bg=self.colors["bg_panel"], fg=self.colors["fg_highlight"])
